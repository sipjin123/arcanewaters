...
public class Util : MonoBehaviour {
   ...
   public static bool isWeakAgainst (Element outgoingElement, Battler battler) {
      // Determines if the battler is weak against the element
      switch (outgoingElement) {
         case Element.Air:
            if (battler.getBattlerData().airDefenseMultiplier < 0) {
               return true;
            }
            break;
         case Element.Fire:
            if (battler.getBattlerData().fireDefenseMultiplier < 0) {
               return true;
            }
            break;
         case Element.Water:
            if (battler.getBattlerData().waterDefenseMultiplier < 0) {
               return true;
            }
            break;
         case Element.Earth:
            if (battler.getBattlerData().earthDefenseMultiplier < 0) {
               return true;
            }
            break;
      }
