...
namespace NubisTranslator {
   public class Fetch_Inventory_v1Controller {
      public static string userInventory (int usrId, int currentPage, int category, int weaponId, int armorId, int hatId) {
         ...
         if (!hasItemFilter) {
            itemFilterContent = "and (itmCategory = " + (int) Item.Category.Weapon + " or itmCategory = " + (int) Item.Category.Armor + " or itmCategory = " + (int) Item.Category.Hats + " or itmCategory = " + (int) Item.Category.CraftingIngredients + ")";
         }
