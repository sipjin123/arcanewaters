         ...
         for (int i = 0; i < editorSize.x; i++) {
            for (int j = 0; j < editorSize.y; j++) {
               ...
               for (int k = midLayers.Count - 1; k >= 0; k--) {
                  ...
                  // If the is a ceiling tile and it's bound by 2 doorframe tiles, handle differently
                  if (midLayers[k].layer.CompareTo("ceiling") == 0) {
                     bool bound = false;
                     if (i > 0 && i < editorSize.x - 1 && j > 0 && j < editorSize.y - 1) {
                        for (int z = 0; z < midLayers.Count; z++) {
                           if (midLayers[z].layer.Equals("door-frame")) {
                              TileWithCollision left = midLayers[z].tileMatrix[i - 1, j];
                              TileWithCollision right = midLayers[z].tileMatrix[i + 1, j];
                              TileWithCollision top = midLayers[z].tileMatrix[i, j + 1];
                              TileWithCollision bot = midLayers[z].tileMatrix[i, j - 1];

                              if (left != null && right != null && left.collisionType == TileCollisionType.CancelDisabled && right.collisionType == TileCollisionType.CancelDisabled) {
                                 bound = true;
                                 break;
                              }
                              if (bot != null && top != null) {
                                 if (bot.collisionType == TileCollisionType.Enabled && top.collisionType == TileCollisionType.Enabled) {
                                    ...
                                    // Move the ceiling tile to doorframe layer to place it on top, don't place a collider to it
                                    midLayers[z].tileMatrix[i, j] = midLayers[k].tileMatrix[i, j];

                                    // Bellow it, place a hidden doorframe tile to extend the collider
                                    midLayers[k].tileMatrix[i, j] = new TileWithCollision {
